      ...
      <section>
        {context.i.map((a: string | number, i: React.Key | null | undefined) => (
          <div key={i} className="container-info">
            {context.dates[a]?.id ? (
              <>
                <p className="p type">{context.dates[a]?.bodyType}</p>
                <h3>
                  {context.dates[a]?.modelName}{" "}
                  <p className="p">{context.dates[a]?.modelType}</p>
                </h3>
                <Image
                  className="pic"
                ...
                <div className="container-menu">
                  <div className="menus">
                    <Link href={`/learn/${context.dates[a]?.id}`} target="_blank">
                      LEARN
                    </Link>
                    <Image
                      src={arrow}
                  ...
                  <div className="menus">
                    <Link href={`/shop/${context.dates[a]?.id}`} target="_blank">
                      SHOP
                    </Link>
                    <Image
                      src={arrow}
